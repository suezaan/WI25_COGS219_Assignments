...
Goal: will plot the size of a certain brain functional network relative to total cortex surface area (%) (e.g., Default Mode Network takes up 10% of total cortex surface area)

End result: plots that show how different are TS vs. non-TS subjects in terms of functional network sizes? 
...
p <- ggplot(data_long, aes(x = Group, y = Value, color = Network)) +
  ...
  facet_wrap(~ Network, scales = "fixed", ncol = 4) +  
  labs(
    x = "Group",
    y = "% Cortex",
...
Addressed weaknesses 
  ...
  1. Y-axis range is not identical in "Salience" network plot, so there's a chance that it might exaggerate the group difference 
  -> used only one y-axis with 0~20% range 
